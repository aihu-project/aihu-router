...
for (const marker of ['NPM_CONFIG_USERCONFIG', 'NPM_CONFIG_GLOBALCONFIG', '_authToken', '_auth', '_password'])
...
for (const field of ['exports', 'main', 'module', 'types', 'dependencies', 'peerDependencies', 'optionalDependencies', 'os', 'cpu', 'libc'])
  if (!pack.includes(`'${field}'`)) throw new Error(`pack contract omits manifest field ${field}`)
if (repo === 'aihu-compiler' && (/workflow_dispatch:/.test(workflow) || /wasm-pack\.github/.test(workflow)))
